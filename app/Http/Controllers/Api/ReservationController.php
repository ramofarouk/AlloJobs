...
public function show(Request $request, $idUser)
{
    return response()->json(
        ReservationResource::collection(Reservation::where('user_id', '=', $idUser)->orderBy('created_at','DESC')->get())
    );
}
     ...
     *
     */
     public function details(Request $request, $idReservation)
     {

        $args = array();
        $course = Reservation::where('id', '=', $idReservation)->first();
        $args['course'] = new ReservationResource($course);
